  ...
  if (searchQuery.split(" ").join("").length <= 0) {
    ...
    Object.keys(vaultContents.accounts).forEach((accountID) => {
      ...
      const accountElement = document.querySelector(
        "div.tab.all_accounts > ul.accounts > div.account#accountid_" +
          accountID
      );
      ...
      if (val.name.toLowerCase().indexOf(searchQuery.toLowerCase()) > -1) {
        ...
        getListOfObjValsRecursive(val.properties).forEach((propObj) => {
          ...
          if (propName.toLowerCase().indexOf(searchQuery.toLowerCase()) > -1) {
            ...
            propVal.toLowerCase().indexOf(searchQuery.toLowerCase()) > -1
          ) {
            isSearchResult = true;
          }
        });
    ...
    if (
      document.querySelectorAll(
        "div.tab.all_accounts > ul.accounts > div.account.displayed"
      ).length <= 0
    ) {
      document
        .querySelector("div.tab.all_accounts")
        .classList.add("no_results");
    } else {
      document
        .querySelector("div.tab.all_accounts")
        .classList.remove("no_results");
    }
  }
};

// search bar element
const searchBarInputElement = document.querySelector(
  "div.tab.all_accounts > nav > div.row > input[type=text]"
);
...
searchBarInputElement.addEventListener("blur", () => {
  document.querySelector("div.tab.all_accounts").classList.remove("searching");
});
