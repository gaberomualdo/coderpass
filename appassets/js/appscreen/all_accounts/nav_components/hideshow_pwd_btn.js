...
const eventHideShowPasswordBtn = (fromMenu = false) => {
  if (!fromMenu) {
    ipcRenderer.sendSync('menu-toggle-hide-pwd', '');
  }
...
hideShowPasswordBtnElement.addEventListener('click', () => {
  eventHideShowPasswordBtn();
});

ipcRenderer.on('menu-toggle-hide-pwd', () => {
  eventHideShowPasswordBtn(true);
});
