...
window.addEventListener("scroll", () => {
  ...
  const navHasNotTopClass = document
    .querySelector("div.tab.all_accounts > nav")
    .classList.contains("not_top");
  if (scrollPixelsFromTop > initialNavPixelsFromTop) {
    if (!navHasNotTopClass) {
      document
        .querySelector("div.tab.all_accounts > nav")
        .classList.add("not_top");
    }
  } else if (navHasNotTopClass) {
    document
      .querySelector("div.tab.all_accounts > nav")
      .classList.remove("not_top");
  }
});
window.addEventListener("load", () => {
  initialNavPixelsFromTop = document
    .querySelector("nav")
