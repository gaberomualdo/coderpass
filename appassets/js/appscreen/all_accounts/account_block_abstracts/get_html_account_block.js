...
const getHTMLOfPropertiesSection = (properties) => {
  ...
  const addPropertyToPropertiesHTML = (propertyName, propertyValue) => {
    propertiesHTML +=
      "<li><p class='name'>" +
      propertyName +
      ":</p><p class='value default_style_code'><span class='password'>" +
      propertyValue +
  ...
  Object.keys(properties).forEach((propertyName) => {
    ...
    if (typeof propertyValue == "object") {
      propertiesHTML +=
        "<li class='subobj_label' onclick='this.classList.toggle(\"open\"); this.nextSibling.classList.toggle(\"open\");'><p class='name'>" +
        propertyName +
        ":</p></li>";
      propertiesHTML += "<ul>";
